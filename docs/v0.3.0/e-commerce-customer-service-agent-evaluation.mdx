...
<Steps>
  ...
  <Step title="Start the agent server">
    <Tabs>
      ...
      <Tab title="Custom">
        No server needed. The custom agent connector loads `custom_agent.py` directly as a Python class at runtime. Skip this step and proceed to run simulation.
      </Tab>
    </Tabs>
  </Step>
  <Step title="Run simulation and evaluation">
    ...
    <Tabs>
      ...
      <Tab title="Custom">
        No separate server terminal is needed. From `examples/e-commerce`:
        ...
        Or run the full pipeline as a Python script:
...
- **Custom:** Subclass `BaseAgent` and point `config_custom.yaml` at your module. See [Custom agent configuration](/simulate-conversation#connection-types) for details.

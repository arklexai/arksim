...
qualitative_thresholds:
  agent_behavior_failure: "no_failure"
```
...
A single float (0–1) applied to every conversation's `overall_agent_score`. Any conversation below the threshold fails the run.
...
Per-metric minimum scores on each metric's native scale. The mean score across all turns per conversation is compared against the threshold. Every conversation must meet it.
...
Hard-gate label requirements for qualitative metrics. Every evaluated turn in every conversation must return the required label. Turns where the metric did not run are skipped.

```yaml
qualitative_thresholds:
  agent_behavior_failure: "no_failure"
  prohibited_statements: "clean"   # example custom qualitative metric
```

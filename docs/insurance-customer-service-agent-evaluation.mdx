...
<CardGroup cols={2}>
  ...
  <Card icon="diagram-project" title="Option 2: OpenAI Agents SDK RAG">
    RAG-based agent using the OpenAI Agents SDK with an insurance knowledge base. Supports A2A or Chat Completions; use `config_a2a.yaml` or `config_chat_completions.yaml`.
  </Card>
</CardGroup>
...
<Steps>
  ...
  <Step title="Run simulation and evaluation">
    In a new terminal, run from the `examples/bank-insurance` directory. Set the key that matches your interface (AGENT_API_KEY for Chat Completions, A2A_API_KEY for A2A), then run with the matching config:

...
| `numeric_thresholds`        | dict[string, float] \| null | Per-metric thresholds on each metric's native scale. Keys are metric names; values are minimum required scores. Built-in turn-level metrics use a 1–5 scale (mean across turns per conversation). `goal_completion` is stored as 0–1 and compared directly. |
...
| `1`  | `EXIT_EVAL_FAILED` | Evaluation completed but one or more threshold checks failed (`score_threshold`, `numeric_thresholds`, or `qualitative_failure_labels`). |
...
| `4`  | `EXIT_NETWORK_ERROR`  | Network or LLM provider unreachable (connection error or timeout).               |

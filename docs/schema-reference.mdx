...
| `qualitative_failure_labels` | dict[string, list[string]] \| null | Hard-gate failure labels for qualitative metrics. Keys are metric names; values are lists of labels that trigger failure (e.g. `agent_behavior_failure: [false_information]`). Any evaluated turn whose label appears in the list fails the run with exit code 1. |

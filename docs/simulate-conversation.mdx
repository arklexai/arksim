...
| -------- | ----------- |
| `scenario.agent_context` | Context about the agent/company the user is talking to. |
| `scenario.goal` | The simulated user's goal (from the scenario). |
| `scenario.knowledge` | The scenario's knowledge items (background information the user has). |
| `simulation.profile` | The generated persona text (from the scenario's user attributes). |

Use them in your template with `{{ scenario.goal }}`, `{{ simulation.profile }}`, and so on. If you omit `simulated_user_prompt_template`, the default prompt is used.

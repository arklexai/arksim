...
<ResponseField name="scenarios" type="list[dict]" required>
  ...
  <Expandable title="scenario fields">
    ...
    <ResponseField name="user_profile" type="string">
      A natural-language persona description for the simulated user. This is used directly in the simulation prompt to shape tone, communication style, and behavior.

      Write in second person (e.g. "You are Alex who is conscientious, introverted, and agreeable. You are a 30-year-old engineer from Chicago...").
    </ResponseField>
    ...
    <ResponseField name="origin" type="dict[string, Any]">
      Freeform metadata used to record how a scenario was generated. Not used by the simulator and exists purely for provenance tracking.
      ...
      - `user_attributes`: Structured key-value map of the user's traits (demographic, personality, behavioral). Stored here as metadata for traceability and display in reports.
    </ResponseField>
  </Expandable>
</ResponseField>
...
You can write scenario files manually. The minimum required fields are `scenario_id`, `goal`, and `origin`:

...
Results go to `evaluation.json`: **conversations** (one per run), **unique_errors** (deduplicated failures), and **error_scenario_groups** (which scenarios triggered each error).
...
<Expandable title="By error scenario group">
  Each **`error_scenario_groups[]`** entry maps one unique error to the scenarios that triggered it:

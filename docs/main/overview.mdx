...
<AccordionGroup>
  ...
  <Accordion title="How do I get started?">
    Install with `pip install arksim`, run `arksim init` to scaffold a starter config and agent file, then run `arksim simulate-evaluate config.yaml`. See the [quickstart guide](https://docs.arklex.ai/main/quickstart) for a full walkthrough.
  </Accordion>
  <Accordion title="What agents and frameworks are supported?">
    ArkSim works with any AI agent regardless of framework. Connect through a Chat Completions HTTP endpoint, the A2A protocol, or load a Python agent class directly with no server needed. See the full list of [supported integrations](https://docs.arklex.ai/main/integrations).
  </Accordion>
  <Accordion title="Can I run this in CI/CD?">
    Yes. ArkSim runs as a CLI command that exits non-zero when quality thresholds are not met. Add it to any CI pipeline as a quality gate on every pull request. See the [CI integration guide](https://docs.arklex.ai/main/ci-integration).
  </Accordion>
  <Accordion title="What metrics are available?">
    Built-in metrics include helpfulness, coherence, relevance, faithfulness, verbosity, goal completion, and agent behavior failure detection. You can also define custom quantitative and qualitative metrics with full access to the conversation context. See the [evaluation guide](https://docs.arklex.ai/main/evaluate-conversation).
  </Accordion>
</AccordionGroup>

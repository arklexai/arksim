...
1. **Explicit**: Return `AgentResponse` with tool calls from `execute()`. Works with any connector.
...
| A2A | Supported | Planned |
| Chat Completions | Supported | Planned |

<Info>
Automatic capture currently works with the Python connector only. A2A and Chat Completions support is planned. All connectors support the explicit `AgentResponse` path.
</Info>
...
No extra config needed. This works with all connector types.
...
These keys are always present regardless of which capture method you use.

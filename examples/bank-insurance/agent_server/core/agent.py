"""RAG-powered customer service agent using the OpenAI Agents SDK."""
...
async def retrieve_context(query: str) -> str:
    ...
    Returns:
        Relevant document excerpts, separated by dividers, or a fallback message.
    """
    ...
    parts: list[str] = []
    for r in results:
        header = r["title"] or r["source"] or "Document"
        parts.append(f"[{header}]\n{r['content']}")
    return "\n\n---\n\n".join(parts)


SYSTEM_INSTRUCTIONS = (
    "You are a customer service agent for a bank and insurance company. "
    "Before answering, always call the retrieve_context tool with a concise "
    "standalone version of the customer's question to look up relevant information. "
    "Keep your final response concise & short and direct to the point."
)

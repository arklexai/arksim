...
with open("./examples/bank-insurance/knowledge.json") as f:
    knowledge_config = json.load(f)["knowledge"]
documents_dir = os.path.join("./examples/bank-insurance")
...
class Agent:
    def __init__(self, context_id: str | None = None) -> None:
        ...
        self.system_prompt = (
            "You are a customer service agent. Based on the conversation history and "
            "the provided context, generate a helpful and accurate reply to the user. "
            "Your response must be concise (no more than 40 words). No prefix "
            "(such as 'Assistant:', 'AI: ') is needed. Directly answer the user's question.\n\n"
            "Context: {context}"
        )
        self.gen_question_prompt = PromptTemplate(
            template=(
                "Given a chat history, formulate a standalone question which can be "

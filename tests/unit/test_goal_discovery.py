...
class TestConversationExtractor:
    ...
    def _fact_json(self, attribute: str, value: str, confidence: float = 0.9) -> dict:
        return {
            "attribute": attribute,
            "value": value,
            "confidence": confidence,
            "provenance_span": value,
        }

    @patch("arksim.scenario.goal_discovery.extractor.LLM")
    def test_extract_all_returns_one_result_per_conversation(
        ...
        mock_llm.call_async = AsyncMock(
            return_value='[{"attribute":"goal","value":"track order","confidence":0.9,"provenance_span":"track order"}]'
        )
        mock_llm_cls.return_value = mock_llm

        extractor = ConversationExtractor()
        convs = [self._make_conv(f"message {i}") for i in range(3)]
        results = extractor.extract_all(convs)

        assert len(results) == 3

    @patch("arksim.scenario.goal_discovery.extractor.LLM")
    def test_extract_all_parses_facts(self, mock_llm_cls: MagicMock) -> None:
        from arksim.scenario.goal_discovery.extractor import ConversationExtractor
        from arksim.scenario.goal_discovery.models import ExtractionFact

        raw = '[{"attribute":"goal","value":"fix AC","confidence":0.95,"provenance_span":"AC isn\'t cooling"}]'
        mock_llm = MagicMock()
        mock_llm.call_async = AsyncMock(return_value=raw)
        mock_llm_cls.return_value = mock_llm

        extractor = ConversationExtractor()
        results = extractor.extract_all([self._make_conv("My AC isn't cooling")])

        assert len(results[0].facts) == 1
        fact = results[0].facts[0]
        assert isinstance(fact, ExtractionFact)
        assert fact.attribute == "goal"
        assert fact.value == "fix AC"
        assert fact.confidence == 0.95
        assert fact.provenance_span == "AC isn't cooling"

    @patch("arksim.scenario.goal_discovery.extractor.LLM")
    def test_extract_all_uses_session_id_as_source_id(
        self, mock_llm_cls: MagicMock
    ) -> None:
        from arksim.scenario.goal_discovery.extractor import ConversationExtractor

        mock_llm = MagicMock()
        mock_llm.call_async = AsyncMock(return_value="[]")
        mock_llm_cls.return_value = mock_llm

        extractor = ConversationExtractor()
        conv = self._make_conv("hello", session_id="sess-42")
        results = extractor.extract_all([conv], source_id_key="session_id")

        assert results[0].source_id == "sess-42"
        assert results[0].facts[0].source_id == "sess-42" if results[0].facts else True

    @patch("arksim.scenario.goal_discovery.extractor.LLM")
    def test_extract_all_falls_back_to_index_when_key_missing(
        self, mock_llm_cls: MagicMock
    ) -> None:
        from arksim.scenario.goal_discovery.extractor import ConversationExtractor
        ...
        extractor = ConversationExtractor()
        conv = self._make_conv("hello")  # no session_id in meta
        results = extractor.extract_all([conv], source_id_key="session_id")

        assert results[0].source_id == "0"

    @patch("arksim.scenario.goal_discovery.extractor.LLM")
    def test_extract_all_tolerates_invalid_json(self, mock_llm_cls: MagicMock) -> None:
        from arksim.scenario.goal_discovery.extractor import ConversationExtractor

        mock_llm = MagicMock()
        mock_llm.call_async = AsyncMock(return_value="not valid json {{")
        mock_llm_cls.return_value = mock_llm

        extractor = ConversationExtractor()
        results = extractor.extract_all([self._make_conv("anything")])
...
class TestLLMHeavyGoalDiscovery:
    """Tests for stage 2 canonicalization in LLMHeavyGoalDiscovery."""

    def _make_extraction_result(
        self, source_id: str, goal_value: str, provenance: str = ""
    ) -> object:
    ...
    @patch("arksim.scenario.goal_discovery.llm_heavy.LLM")
    def test_canonicalize_single_batch(self, mock_llm_cls: MagicMock) -> None:
        from arksim.scenario.goal_discovery.llm_heavy import LLMHeavyGoalDiscovery

        mock_llm = MagicMock()
        mock_llm.call_async = AsyncMock(
            return_value='{"canonical": ["Track Order Status", "Cancel an Order"], '
            '"descriptions": ["User tracks their order.", "User cancels order."], '
            '"assignments": [0, 0, 1]}'
        )
        mock_llm_cls.return_value = mock_llm

        results = [
            self._make_extraction_result("c1", "where is my order"),
            self._make_extraction_result("c2", "track my shipment"),
            self._make_extraction_result("c3", "cancel my purchase"),
        ]
        discovery = LLMHeavyGoalDiscovery()
        result = discovery.canonicalize(results, n_input=3)

        assert result.method == "llm_heavy"
        assert result.n_input == 3
        names = {g.name for g in result.goals}
        assert "Track Order Status" in names
        assert "Cancel an Order" in names

    @patch("arksim.scenario.goal_discovery.llm_heavy.LLM")
    def test_canonicalize_prevalence_sums_to_one(self, mock_llm_cls: MagicMock) -> None:
        from arksim.scenario.goal_discovery.llm_heavy import LLMHeavyGoalDiscovery

        mock_llm = MagicMock()
        mock_llm.call_async = AsyncMock(
            return_value='{"canonical": ["Goal A", "Goal B"], '
            '"descriptions": ["Desc A.", "Desc B."], '
            '"assignments": [0, 0, 1, 1]}'
        )
        mock_llm_cls.return_value = mock_llm

        results = [self._make_extraction_result(str(i), f"goal {i}") for i in range(4)]
        discovery = LLMHeavyGoalDiscovery()
        result = discovery.canonicalize(results, n_input=4)

        total = sum(g.prevalence for g in result.goals)
        assert abs(total - 1.0) < 1e-9

    @patch("arksim.scenario.goal_discovery.llm_heavy.LLM")
    def test_canonicalize_empty_extraction_returns_empty(
        self, mock_llm_cls: MagicMock
    ) -> None:
        from arksim.scenario.goal_discovery.llm_heavy import LLMHeavyGoalDiscovery
        from arksim.scenario.goal_discovery.models import ConversationExtractionResult

        results = [ConversationExtractionResult(source_id="c1", facts=[])]
        discovery = LLMHeavyGoalDiscovery()
        result = discovery.canonicalize(results, n_input=1)

        assert result.goals == []
        assert result.method == "llm_heavy"

    @patch("arksim.scenario.goal_discovery.llm_heavy.LLM")
    def test_canonicalize_goal_source_is_llm_extraction(
        self, mock_llm_cls: MagicMock
    ) -> None:
        from arksim.scenario.goal_discovery.llm_heavy import LLMHeavyGoalDiscovery
        ...
        mock_llm = MagicMock()
        mock_llm.call_async = AsyncMock(
            return_value='{"canonical": ["Track Order Status"], '
            '"descriptions": ["User tracks order."], '
            '"assignments": [0, 0]}'
        )
        mock_llm_cls.return_value = mock_llm
        ...
        results = [
            self._make_extraction_result("c1", "where is my order"),
            self._make_extraction_result("c2", "order status check"),
        ]
        discovery = LLMHeavyGoalDiscovery()
        result = discovery.canonicalize(results, n_input=2)

        assert all(g.source == "llm_extraction" for g in result.goals)

    @patch("arksim.scenario.goal_discovery.llm_heavy.LLM")
    def test_canonicalize_tolerates_invalid_json_from_llm(
        self, mock_llm_cls: MagicMock
    ) -> None:
        from arksim.scenario.goal_discovery.llm_heavy import LLMHeavyGoalDiscovery

        mock_llm = MagicMock()
        mock_llm.call_async = AsyncMock(return_value="not json {{")
        mock_llm_cls.return_value = mock_llm

        results = [
            self._make_extraction_result("c1", "track my order"),
            self._make_extraction_result("c2", "track my order"),
        ]
        discovery = LLMHeavyGoalDiscovery()
        result = discovery.canonicalize(results, n_input=2)

        # Fallback uses raw goal values; pipeline must not raise
        assert len(result.goals) >= 1

    @patch("arksim.scenario.goal_discovery.llm_heavy.LLM")
    def test_canonicalize_metadata_fields(self, mock_llm_cls: MagicMock) -> None:
        from arksim.scenario.goal_discovery.llm_heavy import LLMHeavyGoalDiscovery
        ...
        results = [
            self._make_extraction_result("c1", "track my order"),
            self._make_extraction_result("c2", "order tracking"),
        ]
        discovery = LLMHeavyGoalDiscovery()
        result = discovery.canonicalize(results, n_input=2)

        assert result.metadata["n_goal_facts"] == 2
        assert result.metadata["n_canonical"] == 1
        assert result.metadata["n_conversations_with_goals"] == 2
...
class TestIsNegativeEmotion:
    ...
    @patch("arksim.scenario.goal_discovery.llm_heavy.LLM")
    def test_llm_heavy_counts_negative_emotion(self, mock_llm_cls: MagicMock) -> None:
        from arksim.scenario.goal_discovery.llm_heavy import LLMHeavyGoalDiscovery
        from arksim.scenario.goal_discovery.models import (
            ConversationExtractionResult,
            ExtractionFact,
        ...
        mock_llm = MagicMock()
        mock_llm.call_async = AsyncMock(
            return_value='{"canonical": ["Return an Order"], '
            '"descriptions": ["User wants to return."], '
            '"assignments": [0, 0, 0]}'
        )
        mock_llm_cls.return_value = mock_llm

        def _fact(sid: str, goal: str, provenance: str) -> ConversationExtractionResult:
            return ConversationExtractionResult(
                source_id=sid,
                facts=[
                    ExtractionFact(
        ...
        results = [
            _fact("c1", "return item", "I am frustrated, need to return this"),
            _fact("c2", "return product", "I want to return my order"),
            _fact("c3", "return item", "This is terrible, I hate this product"),
        ]
        discovery = LLMHeavyGoalDiscovery()
        result = discovery.canonicalize(results, n_input=3)

        goal = result.goals[0]
        assert goal.negative_emotion_count == 2
